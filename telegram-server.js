/**
 * telegram-server.js v15.0 — Server Telegram Bot + Mini App WebView
 *
 * MỚI v15.0 — Chuyển từ Puppeteer server-side sang Mini App WebView:
 *
 *  ✅ /miniapp — Serve miniapp.html (WebView tự động inject JS + click)
 *  ✅ doVerifyId() → gửi nút "Mở trang xác thực" (web_app button)
 *  ✅ handleUpdate xử lý web_app_data (Telegram.WebApp.sendData)
 *  ✅ /webapp-data POST endpoint dự phòng
 *  ✅ Puppeteer fallback giữ nguyên khi không có MINIAPP_URL
 *
 * CŨ v14.3 — Đồng bộ hoàn toàn JS với BackgroundWebVerifier.java:
 *
...
const MINIAPP_URL    = (process.env.MINIAPP_URL || process.env.RENDER_EXTERNAL_URL || '').replace(/\/+$/, '') + '/miniapp';
...
/* ══════════════════════════════════════════════════════════════════════════════
   ...
   Ưu tiên Mini App WebView (không cần Puppeteer server-side)
   Fallback: Puppeteer → HTTP
   ══════════════════════════════════════════════════════════════════════════════ */
async function doVerifyId(chatId, accountId) {
  ...
  // ── Chế độ Mini App (ưu tiên nếu có URL) ──
  const miniUrl = MINIAPP_URL;
  if (miniUrl && miniUrl.startsWith('http') && !miniUrl.includes('undefined')) {
    const fullUrl = `${miniUrl}?id=${encodeURIComponent(accountId)}`;
    await sendMsg(chatId,
      `⏳ <b>Đang xác thực ID:</b> <code>${accountId}</code>\n\n` +
      `📱 <b>Nhấn nút bên dưới để mở trang xác thực.</b>\n` +
      `<i>WebView sẽ tự động điền ID và click qua 4 bước.</i>`,
      {
        reply_markup: {
          inline_keyboard: [[
            { text: '🌐 Mở trang xác thực', web_app: { url: fullUrl } }
          ]]
        }
      }
    );
    // Chờ kết quả từ Mini App qua /webapp-data hoặc webhook web_app_data
    // (Bot nhận web_app_data tự động qua webhook handleUpdate bên dưới)
    return;
  }
...
async function handleUpdate(update) {
  /* ── Nhận kết quả từ Mini App (Telegram.WebApp.sendData) ── */
  if (update && update.message && update.message.web_app_data) {
    const msg2  = update.message;
    const cId   = msg2.chat && msg2.chat.id;
    const raw   = msg2.web_app_data.data;
    if (cId && raw) {
      try {
        const d = JSON.parse(raw);
        if (d.status === 'granted' && d.id && /^\d{6,15}$/.test(String(d.id))) {
          const secs = parseInt(d.expires) || 3600;
          console.log(`[MiniApp] web_app_data granted: chatId=${cId} id=${d.id} expires=${secs}s`);
          await _handleSuccess(String(cId), String(d.id), secs);
        } else {
          await sendMsg(cId, '❌ <b>Xác thực thất bại.</b>\n<i>Mini App không trả về kết quả hợp lệ.</i>\n\nThử lại bằng cách gửi ID.');
        }
      } catch(e) {
        console.error('[MiniApp] parse web_app_data lỗi:', e && e.message);
      }
    }
    return;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (reqPath === '/miniapp') {
    ...
    try {
      ...
      res.writeHead(404); res.end('miniapp.html not found');
    }
    return;
  }

  /* ── Nhận kết quả từ Mini App (sendData) ── */
  if (reqPath === '/webapp-data' && req.method === 'POST') {
    ...
    readBody(req).then(async body => {
      try {
        ...
        const expires   = parseInt(body.expires) || 3600;
        if (chatId && accountId && /^\d{6,15}$/.test(String(accountId))) {
          console.log(`[MiniApp] Nhận granted: chatId=${chatId} ID=${accountId} expires=${expires}s`);
          await _handleSuccess(String(chatId), String(accountId), expires);
        }
      } catch(e) { console.error('[MiniApp] webapp-data lỗi:', e && e.message); }
    }).catch(() => {});
    return;
  }

  if (reqPath === '/webhook' && req.method === 'POST') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true }));
    try {
...
async function start() {
  server.listen(PORT, '0.0.0.0', async () => {
    console.log(`\n🤖 Telegram Server v15.0 đang chạy tại port ${PORT}`);
    console.log(`   BOT_TOKEN  : ${BOT_TOKEN.slice(0, 10)}...`);
    console.log(`   KEY_SERVER : ${KEY_SERVER_URL}`);
    console.log(`   KEY_APP_ID : ${KEY_APP_ID}`);
    console.log(`   MINIAPP_URL : ${MINIAPP_URL || '(chưa set — dùng Puppeteer fallback)'}`);
    console.log(`   Flow: Mini App WebView → JS inject → web_app_data`);
