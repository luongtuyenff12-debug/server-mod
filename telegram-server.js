...
/* ── Kiểm tra lỗi navigation (context bị huỷ do redirect/navigate) ─────── */
...
/* ── safeEvaluate: tự đợi trang ổn định rồi evaluate, retry tối đa 3 lần ─ */
async function safeEvaluate(page, fn, fallback, retries) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (_isNavError(e)) {
        // Đợi trang ổn định sau navigation
        await _pgSleep(1500);
        try {
      ...
      console.warn(`[safeEvaluate] lần ${i+1} lỗi:`, e.message);
...
/* ── Đợi trang ổn định (không còn network request hoặc timeout) ─────────── */
async function _waitStable(page, timeout) {
...
async function _pgFillInput(page, accountId) {
  ...
  for (const sel of selectors) {
    try {
      ...
      if (_isNavError(e)) { await _pgSleep(1200); continue; }
    }
  }
  // Fallback: điền bằng evaluate thuần
  return await safeEvaluate(page, (val) => {
    const inputs = Array.from(document.querySelectorAll('input'));
    for (const inp of inputs) {
      if (['hidden','checkbox','radio','submit','button'].includes(inp.type)) continue;
      try {
        const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
        if (setter) setter.call(inp, val);
        ['input','change','keyup'].forEach(ev => inp.dispatchEvent(new Event(ev, { bubbles: true })));
...
async function _pgClickCWD(page) {
  ...
  // Thử click, nếu navigation xảy ra thì đợi trang load xong rồi thử lại
  for (let attempt = 0; attempt < 3; attempt++) {
    const result = await safeEvaluate(page, CLICK_FN, false, 2);
    if (result) {
      // Click thành công — đợi trang ổn định (có thể navigate)
      await _waitStable(page, 5000);
      return true;
    }
    if (attempt < 2) await _pgSleep(800);
  }
  return false;
}
...
async function _pgClickGeneric(page) {
  ...
  const result = await safeEvaluate(page, GENERIC_FN, false, 2);
  if (result) await _waitStable(page, 5000);
}

async function _pgGetState(page) {
  const STATE_FN = () => {
    const html = document.body ? document.body.innerHTML : '';
    const granted = /access\s*granted/i.test(html);
    let expires = -1;
    if (granted) {
  ...
  return await safeEvaluate(page, STATE_FN, { granted: false, expires: -1, countdown: 0 }, 3);
}
...
async function _runWithPuppeteer(chatId, accountId, puppeteer, chromium) {
  ...
  try {
    ...
    // Chặn quảng cáo & resource không cần thiết
    page.on('request', req => {
    ...
    // Lắng nghe navigation để log (giúp debug nếu còn lỗi)
    ...
    // Bước 0: Tải trang, đợi domcontentloaded rồi thêm 1.5s cho JS render
    ...
    // Click CWD (nút "Continue Without Discord") — _pgClickCWD đã tự đợi stable sau click
    await _pgClickCWD(page);
    await _pgSleep(1000);

    let expiresSecs = -1, grantedFound = false;
    for (let step = 2; step <= 5 && !grantedFound; step++) {
      // Đảm bảo trang đã ổn định trước khi evaluate
      ...
      if (state.countdown > 0) {
        ...
        const cntResult = await _pgWaitCountdown(page, wait * 1000);
        if (cntResult === 'granted') {
          const st2 = await _pgGetState(page);
          grantedFound = true; expiresSecs = st2.expires; break;
        }
      }

      await sendMsg(chatId, `🖱 <b>Bước ${step}/4:</b> Click tiếp tục...\n🆔 ID: <code>${accountId}</code>`);

      // _pgClickCWD đã tự đợi navigation sau click
      const clicked = await _pgClickCWD(page);
      if (!clicked) {
        await _pgClickGeneric(page);
        await _pgSleep(800);
      }

      // Đợi thêm sau click để trang render xong
      await _pgSleep(1500);
    ...
    if (_isNavError(e)) {
      await sendMsg(chatId,
        `⚠️ <b>Trình duyệt gặp navigation bất ngờ.</b>\n<code>${String(e && e.message || e).slice(0,120)}</code>\n\n` +
        `↩️ Gửi lại ID để thử lại.`
      );
    } else {
      await sendMsg(chatId,
