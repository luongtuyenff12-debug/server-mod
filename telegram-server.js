/**
 * telegram-server.js v14.1 — Server Telegram Bot độc lập
 *
 * FIX v14.1 — Sửa bug "Bước 1 → thất bại ngay":
 *
 *  ✅ FIX CHÍNH: Poll sau bước 1 tăng lên 3 PHÚT (không phải 12s)
 *       Vòng poll chính xử lý đủ 4 trạng thái: step1/unlocking/unlocked/granted
 *
 *  ✅ FIX: step1 sau click CWD → không thoát poll mà xử lý tiếp:
 *       • Nếu còn countdown → đợi countdown hết rồi mới click
 *       • Nếu không có countdown mà click miss → retry điền ID + click lần 2
 *
 *  ✅ FIX: unlocking countdown: đọc giá trị thực từ trang, đợi đúng giây + 2s buffer
 *       Tránh đợi cố định 8s khi trang có countdown 5s
 *
 *  ✅ FIX: Đóng tab quảng cáo tự động mở sau mỗi lần click Continue
 *
 *  ✅ FIX: Load trang đợi 3s (React/Vue render) thay vì 2s
 *
 *  ✅ GIỮ NGUYÊN v14.0: _pgGetState, _pgFillInput, _pgFindAndClickContinue,
 *       key auth, session, webhook, anti-sleep, tất cả lệnh bot
 *
 * Biến môi trường:
