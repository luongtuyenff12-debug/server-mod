...
function adminAuth(req, res, next) {
  ...
  const expected = (CONFIG.ADMIN_SECRET || '').trim();
  if (provided !== expected)
    return res.status(401).json({ status: 'UNAUTHORIZED', error: 'Secret không đúng' });
  next();
}

// Ping — check server alive, không cần auth
app.get('/admin/ping', (req, res) => res.json({ ok: true, ts: Date.now() }));
...
<div id="login-screen">
  <div class="login-box">
    ...
    <button id="btn-login" class="btn btn-primary btn-full" onclick="doLogin()">Đăng nhập →</button>
  </div>
</div>
...
async function doLogin() {
  // Lấy secret, trim toàn bộ whitespace (kể cả newline ẩn từ autocomplete)
  const secret = document.getElementById('inp-secret').value.replace(/^\s+|\s+$/g,'');
  const errEl  = document.getElementById('login-err');
  const btn    = document.getElementById('btn-login');
  errEl.style.display = 'none';
  if (!secret) { showErr(errEl,'⚠️ Vui lòng nhập Admin Secret'); return; }

  // Disable button, hiện loading
  btn.disabled = true;
  ...
  try {
    // Bước 1: Ping kiểm tra server alive (timeout 20s — chờ Render wake up)
    btn.textContent = '⏳ Đang khởi động server...';
    try {
      await fetchWithTimeout(SERVER+'/admin/ping', {}, 20000);
    } catch(e) {
      if(e.name==='AbortError') throw new Error('timeout_ping');
      throw new Error('network');
    }

    // Bước 2: Gửi request login thực sự (timeout 10s)
    btn.textContent = '⏳ Đang xác thực...';
    let r;
    try {
      r = await fetchWithTimeout(SERVER+'/admin/keys', {
        headers: { 'x-admin-secret': secret, 'Content-Type': 'application/json' }
      }, 10000);
    } catch(e) {
      if(e.name==='AbortError') throw new Error('timeout_auth');
      throw new Error('network');
    }
    ...
    // Parse JSON an toàn
    let d;
    try { d = await r.json(); } catch { throw new Error('parse_error'); }
    ...
    SECRET = '';
    const msgs = {
      'wrong_secret' : '❌ Sai mật khẩu — kiểm tra lại Admin Secret trên Render',
      'rate_limit'   : '⏳ Quá nhiều yêu cầu, thử lại sau 1 phút',
      'timeout_ping' : '⏳ Server đang khởi động (Render free tier), thử lại sau 30 giây',
      'timeout_auth' : '⏳ Server phản hồi chậm, thử lại ngay',
      'parse_error'  : '❌ Server trả dữ liệu lỗi, thử reload trang',
      'network'      : '❌ Không kết nối được server — kiểm tra mạng',
    };
    showErr(errEl, msgs[e.message] || ('❌ Lỗi: ' + e.message));
  } finally {
    btn.disabled = false;
...
// Nhấn Enter trong ô mật khẩu để login
