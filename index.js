...
function doLogin() {
  ...
  fetch(SERVER+'/admin/keys',{headers:{'x-admin-secret':secret},signal:controller.signal})
    .then(r => {
      clearTimeout(tid);
      if(btn){btn.disabled=false;btn.textContent='Đăng nhập →';}
      if(r.status===401){SECRET='';showErr(errEl,'❌ Sai Admin Secret — kiểm tra lại mật khẩu');return;}
      if(!r.ok){SECRET='';showErr(errEl,'Lỗi server: HTTP '+r.status);return;}
    ...
    .catch(e=>{
      ...
      if(btn){btn.disabled=false;btn.textContent='Đăng nhập →';}
      SECRET='';
      if(e.name==='AbortError'){showErr(errEl,'⏳ Timeout — server đang khởi động, thử lại sau 30 giây');}
      else{showErr(errEl,'Không kết nối được server: '+e.message);}
    });
}
